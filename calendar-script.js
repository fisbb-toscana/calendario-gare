	...
	try {
		...
		rawEvents = [
			...
			},
			{
				...
				"handicap": "NO",
				"arbitri": "SI",
				"vincitore": "Prima categoria: Mario Rossi\nSeconda categoria: Luca Bianchi\nTerza categoria: Paolo Verdi",
				"locandina": "https://trello.com/1/cards/6a32d443140bdfc68e715dfe/attachments/6a32d45b689d3cbf84b01c8f/download/londa_2026.jpg"
			}
		];
...
function switchView(viewType) {
	...
	if (activeBtn) { 
		activeBtn.style.background = '#ffffff'; activeBtn.style.color = '#0f172a'; activeBtn.style.boxShadow = '0 1px 2px rgba(0,0,0,0.05)'; 
	}
...
function navigateCalendar(direction) {
	if (activeViewType === 'year' || activeViewType === 'list') {
		...
		if (currentGridMonth > 11) { 
			currentGridMonth = 0; currentGridYear++; 
		}
		else if (currentGridMonth < 0) { 
			currentGridMonth = 11; currentGridYear--; 
		}
	...
	for (let i = 0; i < 12; i++) {
		...
		if (monthEvents.length === 0) {
			...
			monthEvents.forEach(evt => {
				...
				item.innerHTML = `<span class="event-day">Fin. ${parsedData.day}</span><span class="event-name" title="${evt.title}">${evt.title}</span>`;
